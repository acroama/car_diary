 ...
 * Car Dairy Drop Box
 * 
 * app key : arolbnglpxalj43
 * app secret : xiazimxjjz29sjd
 */

var DROPBOX_APP_KEY = 'arolbnglpxalj43';
...
$(function () {
	...
	if (client.isAuthenticated()) {
		...
		client.getDatastoreManager().openDefaultDatastore(function (error, datastore) {
			...
			diaryTable = datastore.getTable('tasks');
	...
	function setCompleted(id, completed) {
		diaryTable.get(id).set('completed', completed);
	}

	// Delete the record with a given ID.
	function deleteRecord(id) {
		diaryTable.get(id).deleteRecord();
	}
